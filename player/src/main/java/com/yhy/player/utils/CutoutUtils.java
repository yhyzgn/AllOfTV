 ...
 * 刘海屏工具
 * <p>
 * Created on 2023-05-11 11:23
 *
 * @author 颜洪毅
 * @version 1.0.0
 * @since 1.0.0
 */
public abstract class CutoutUtils {
     ...
     * @return 大小
     */
    public static Size size(Context context) {
        Size size = new Size(0, 0);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            DisplayCutout dc = PlayerUtils.scanForActivity(context).getWindow().getDecorView().getRootWindowInsets().getDisplayCutout();
            if (null != dc) {
                int dcLeft = dc.getSafeInsetLeft();
                int dcRight = dc.getSafeInsetRight();
                int dcTop = dc.getSafeInsetTop();
                int dcBottom = dc.getSafeInsetBottom();
                size = new Size(dcLeft + dcRight, dcTop + dcBottom);
            }
        }
        return size;
    }
